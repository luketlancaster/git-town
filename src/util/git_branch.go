...
func EnsureBranchInSync(branchName, errorMessageSuffix string) {
	Ensure(IsBranchInSync(branchName), fmt.Sprintf("'%s' is not in sync with its tracking branch. %s", branchName, errorMessageSuffix))
}
...
func EnsureDoesNotHaveBranch(branchName string) {
	Ensure(!HasBranch(branchName), fmt.Sprintf("A branch named '%s' already exists", branchName))
}

// EnsureHasBranch enforces that a branch with the given name exists
func EnsureHasBranch(branchName string) {
	Ensure(HasBranch(branchName), fmt.Sprintf("There is no branch named '%s'", branchName))
}
...
func EnsureIsNotMainBranch(branchName, errorMessage string) {
	Ensure(!IsMainBranch(branchName), errorMessage)
}
...
func EnsureIsNotPerennialBranch(branchName, errorMessage string) {
	Ensure(!IsPerennialBranch(branchName), errorMessage)
}
...
func HasLocalBranch(branchName string) bool {
	return DoesStringArrayContain(GetLocalBranches(), branchName)
}
